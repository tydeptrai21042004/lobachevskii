...
def tiny_etex_like_data() -> EtexData:
    return EtexData(
        ...
        concentrations=np.ones((6, 4)),
    )


def test_graph_core_and_wave_stability():
    g = build_etex_graph(tiny_etex_like_data(), k=2)
    assert nx.is_connected(g)
    model = build_wave_model(g, max_time=3)
    lam_max = np.linalg.eigvalsh(model.laplacian)[-1]
    assert model.nu_dt**2 * lam_max < 4.0
    assert modal_alpha(model, (1, 2, 3)) > 0.0
...
def test_pena_heat_baseline_runs_on_small_graph():
    ...
    for u, v in g.edges:
        g[u][v]["weight"] = 1.0
    # A localized positive observation is enough to exercise the paper-derived solver.
    b = np.zeros(6)
    b[2] = 1.0
